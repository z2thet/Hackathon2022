...
shinyServer(function(input, output, session) {
  ...
  AE_data <- selected_data <- reactive({ if (is.null(input$data_file)) {
    ...
    if("cdus_toxicity_type_code" %in% names(output)){output$cdus_ctcae_toxicity_type_code <- output$cdus_toxicity_type_code }
    ...
    print("dim(output)")
    print(dim(output))
    output<-as.data.frame(output %>% filter(is.na(grade)==FALSE))
    print(dim(output))
    output
  })
  ...
  output$AEcounttables <- DT::renderDataTable({
    ...
    adverse_events2 <- adverse_events0 %>% filter(is.na(grade)==FALSE) %>%
      group_by(sequence_no, cdus_ctcae_toxicity_type_code) %>%
      filter(grade == max(grade, na.rm = TRUE)) %>%
      ungroup() %>%
      distinct(sequence_no, cdus_ctcae_toxicity_type_code, grade, .keep_all = TRUE)
    
    summary_ae_drug =adverse_events2%>%
      mutate(grade = recode_factor(grade, `1` = "Grade 1", `2` = "Grade 2", `3` = "Grade 3",`4` = "Grade 4",`5` = "Grade 5")) %>%
      count( `Adverse event` = cdus_ctcae_toxicity_type_code,grade) %>%
  ...
  {# 
  # observeEvent(input$single_var_input,{
  #   
  #   subject<-input$single_var_input  
  #   AEearly_Cut <- NA;#input$AEplot_EarlyAECut
  #   print("AEearly_Cut")
  #   print(AEearly_Cut)
  #   TimeDisplay <- FALSE;#input$AEplot_ShowTime
  #   print("TimeDisplay")
  #   print(TimeDisplay)
  ...
  #   print("vvvvvvvvvvv THIS IS THE DATA TO PLOT vvvvvvvvvvvvvvvv")
  #   
  #   print("This is inside the observeEvent thing")
  #   #TEsting
  ...
  single_var_plot_early <- reactive({
   ...
   # print(names(w2))
    ...
    w3 <- merge(  da_data_subject, w2.2early %>% select(on_treatment_date)) %>% mutate(dadaynumber = as.numeric(difftime(start_date_of_drug ,on_treatment_date,units='days')) )
    # print("some DA data w3")
    # print(w3)
    # print("dim DA data w3")
    # print(dim(w3)) 
    ...
    print("vvvvvvvvvvv THIS IS THE DATA TO PLOT vvvvvvvvvvvvvvvv")
    #TEsting
    print("This is inside the reactive thing")
    print("length unique W2$index")
    ...
    print(w3%>% select(sequence_no,start_date_of_drug , drug,dadaynumber))
    print(w3<-w3%>% select(sequence_no,start_date_of_drug , drug,dadaynumber) %>% filter(dadaynumber<=lastAEdaynumber))
...
print("^^^^^^^^^^^^^^^^^^^^^^^^^^ w2 ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
   #g.cols=c("purple","blue",'green','black','gray75','gray25' ,'tan','tan2'  )
      ...
      plot1 <- plot1 +
        ...
        vline.fun(early.AE.status.tmp=early.AE.status,early.time.tmp=early.time)+
        theme(legend.position="top",
              axis.text=element_text(size=14*k),
              axis.title=element_text(size=18*k,face="bold"),
              legend.text = element_text(size=16*k),
              legend.title = element_text(size=18*k),
  ...
  output$rendAEmeasuresAEcatselect <- renderUI({
    
    AE_data <- AE_data()
    AE_Cats <- unique(AE_data[,"toxicity_category"])
    AE_Cats <- c("All AE Categories",AE_Cats)
    selectInput("AEmeasuresAEcatselect","Select AE Category", choices = AE_Cats)
    
  })
  
  output$rendAEmeasuresAEtypeselect <- renderUI({
    ...
    if (input$AEmeasuresAEcatselect != "All AE Categories") {
      ...
      selectInput("AEmeasuresAEtypeselect","Select AE Type", choices = AE_Types)
    }
  ...
  toxicity.whole.summary.data <- reactive({  
    ...
    if(I("last_visit_date" %in% names(toxdata0)) == FALSE){
      ...
      print(" ~~~~~~~~~~~~~~~~~~~~(toxdata0)~~~~~~~~~~~~~~~~~~~~~~~")
    # print(names(toxdata0))
     print(toxdata0 %>% select(pid,off_treatment_date.y,on_treatment_date,
                               last_followup_date,last_visit_date))
  ...
  output$toxicitytableoutput <- DT::renderDataTable({
    ...
    datatable(todisplay, rownames = FALSE, options = list(pageLength = 100)
              # , 
              # options = list(
              #   scrollX = T, autoWidth = TRUE, searchHighlight = TRUE,
              #   initComplete = JS(
              #     "function(settings, json) {",
              #     "$(this.api().table().header()).css({'background-color': '#58b0e3', 'color': '#fff'});",
              #     "}"))
  ...
  kmandcoxphinfofromtoxdata <- eventReactive(input$gogocoxmodels,{  
    ...
    kmandboxplotsfunction <- function(df){
      ...
      { print('~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ next !!!') 
        ...
        { #print("OS ------ OS -----------------OS ______")
          ...
          cox1<-suppressWarnings(coxph(Surv(os_time,os_censor)~y,data=a4))
          # print('OS')
          # print(summary(cox1))
          tmp99.os<-coef(summary(cox1))
          
          if(exists("cox1")==TRUE){rm(cox1)} 
          cox1<-suppressWarnings(coxph(Surv(os_time,os_censor)~AE.bin,data=a4))
          # print('OS.bin-------------------------------------------')
          # print(summary(cox1))
        ...
        { 
          ...
          cox1<-suppressWarnings(coxph(Surv(pfs_time,pfs_censor)~y,data=a4))
          # print('PFS')
          # print(summary(cox1))
          tmp99.pfs<-coef(summary(cox1))
          cox1<-suppressWarnings(coxph(Surv(pfs_time,pfs_censor)~AE.bin,data=a4))
          # print('PFS.bin')
          # print(summary(cox1))
  ...
  output$rendBPKPplotSelection <- renderUI({
    ...
    selectInput("BPKPplotSelection","Select Plot to View:", choices = all_plot_names)
    
    
  })
  
  output$BPKP_PlotOutput <- renderPlot({
  ...
  output$kmcophinfo <- DT::renderDataTable({
    ...
    datatable(todisplay, rownames = FALSE, options = list(pageLength = 72))
  ...
  responsePvaluelistforestplots <- eventReactive(input$goforstplotstests,{
  ...
  output$rendForestplotSelection <- renderUI({
    
    Forestplot_list <- responsePvaluelistforestplots()
    plot_grade_names <- names(Forestplot_list$plot)
    
    selectInput("ForestplotSelection","Select Plot to View:", choices = plot_grade_names)
    
    
  })
  
  output$Forest_PlotOutput <- renderPlot({
  ...
  #testing_BOR_ans   <- reactive(input$goresponsetests,{  
    testing_BOR_ans   <- eventReactive(input$goresponsetests,{ 
    toxdataNOW <- toxicity.whole.summary.data()
  ...
  durationanalysis   <- eventReactive(input$goCorrelationtests,{  
    ...
    {
      x=x%>%mutate(group=paste(factor(p<0.05,level=c(T,F),label=c('Sig','NS')),
    ...
    print("names(AE_data()) looking for cdus_toxicity_type_code and toxicity_category" )
     print(names(AE_data()))
    # print(dim(AE_data()))
  ...
  survivialtempout <- reactive({
    ...
    fun1<-function(my.data.list=coef.list)
    {
      ...
      for(i in 1:length(surv.name))
      {
        ...
        if(dim.index>0)
        {
          e2<-e2%>%add_rownames(var='data.type')%>% 
            mutate(survival=surv.name[i],
                   AE.type=rep(names(my.data.list),each=2),
                   data.type=sub('y.*','Continuous',sub('AE.*','Occurrence',data.type)))%>%
            relocate(AE.type)
    ...
    print("  str(a1,1,1)  ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
      str(a1,1,1)# List of n tibbles  
      print("  str(a1[[1]],1,1)  ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
     str(a1[[1]],1,1)# xx x of length x 
 
    a2<-sapply(a1,fun1,simplify = F)
    print("what is a2 which is the outpot of running fun1 on a1: ")
      str(a2,1,1)
    ...
    # print("what is a31 take only those from a3 that are not dim 0~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
    # str(a31,1,0)
    # 
    ...
    #   print("x$AE.type")
    #   print(x$AE.type)
    #   x=x%>%mutate(group=paste(survival,
    #                            factor(p<0.05,level=c(T,F),label=c('Sig','NS')),
    ...
    #   tapply(as.vector(x$AE.type),x$group,sort)
    # }
    # ,simplify=TRUE) # ,simplify=TRUE on 20221212
    # 
    a5=sapply(a31,function(x)
    {
      x=x%>%mutate(group=paste(survival,factor(p<0.05,level=c(T,F),label=c('Sig','NS')),factor(HR>1,level=c(T,F),label=c('HR>1','HR<1')),sep='_'))
      tapply(as.vector(x$AE.type),x$group,sort)
    },simplify=F)
    ...
    # 
    # a6<-sapply(a5,function(y) as.vector(
    #   sapply(y,function(x) names(table(sub('\\.sum_unique_AE','',
    #                                        sub('\\.occurrence','',sub('\\.fre','',sub('\\.duration','',x))))))
    #          ,simplify = TRUE)),simplify = TRUE)# ,simplify=TRUE on 20221212
    # 
    ...
    # a6.trt<-sapply(a6,function(y) unlist(sapply(y,function(x) {
    #   index1<-grep('treatment',x); 
    # if(length(index1)==0) NULL else
    # {
    #   x<-x[index1];paste(x,collapse='/')
    # }},simplify=TRUE))  ,simplify=TRUE )# ,simplify=TRUE on 20221212
    # 
    ...
    # a6.no_trt<-sapply(a6,function(y) unlist(sapply(y,function(x) {index1<-grep('treatment',x); 
    #                                                                if(length(index1)>0) x<-x[-index1];
    #                                                                paste(x,collapse='/')},simplify = TRUE))# ,simplify=TRUE on 20221212
    #                   ,simplify = TRUE)# ,simplify=TRUE on 20221212
    # 
    # a6.no_trt<-sapply(a6.no_trt,function(y) sapply(y,fun.AE.summary,simplify = TRUE),simplify = TRUE)# ,simplify=TRUE on 20221212
    # 
    ...
    # a7<-sapply(a6,
    #            function(y) sapply(y,
    #                               function(x) paste(sub('grade3','High-Grade',sub('grade12','Low-Grade',
    #                                                                               sub('treatment\\.related','Trt',x))),
    #                                                 collapse='/'),simplify=TRUE),simplify=TRUE)# ,simplify=TRUE on 20221212
    # 
    ...
    # print("table.AE.survival.All.sig.summary ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
    # print(table.AE.survival.All.sig.summary)
    # 
    ...
    # print(head(q2))
    # 
    # print("ROW NAMES OF q1  ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
    # print(rownames(q1))
    # print("ROW NAMES OF q2 ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
    # print(rownames(q2))
    # print(intersect(rownames(q1),rownames(q2)))
    # 
    ...
    save(a1,a31,a5,a6,a7,q1,q2,q3,q31,q5,q6,q11,file="C:\\Users\\thompszj\\Desktop\\datasets.RData")
    ...
    save(a1,a31,a5,a6,a7,q1,q2,q3,q31,q5,q6,q7,q11,q8,file="C:\\Users\\thompszj\\Desktop\\datasetswithq7andq8.RData")
    q8 
  ...
  # Down load toxicity.whole.summary.data()       
  output$survival_summarydownload <- downloadHandler(
  ...
  output$response_summarydownload <- downloadHandler(
